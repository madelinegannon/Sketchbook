#include "MultiDeviceController.h"
#include "k4astaticimageproperties.h"

MultiDeviceController::MultiDeviceController(vector<string> serial_numbers)
{
    this->serial_numbers = serial_numbers;
    num_devices = this->serial_numbers.size();

    setup();
}

void MultiDeviceController::setup()
{
    // hard code chessboard_pattern for now
    chessboard_pattern.height = 6;
    chessboard_pattern.width = 9;
    ...
    secondary_config = get_subordinate_config();

    // Construct all the things that we'll need whether or not we are running with 1 or 2 cameras
    main_calibration = capturer.get_master_device().get_calibration(main_config.depth_mode,
    ...
    background_image = color_to_opencv(background_captures[0].get_color_image());
    output_image = background_image.clone(); // allocated outside the loop to avoid re-creating every time
    background_image.setTo(cv::Scalar(0, 255, 0));

    // Set up transformation / calibration for second device
    if (num_devices == 2)
    {
        ...
        secondary_depth_to_main_color = k4a::transformation(secondary_depth_to_main_color_cal);
    }

    for (int i = 0; i < num_devices; i++)
    {
        capture_color_pixels.push_back(ofPixels());
        capture_color_textures.push_back(ofTexture());
        capture_depth_pixels.push_back(ofShortPixels());
        capture_depth_textures.push_back(ofTexture());
        capture_ir_pixels.push_back(ofShortPixels());
        capture_ir_textures.push_back(ofTexture());
        colorized_depth_pixels.push_back(ofPixels());
        colorized_depth_textures.push_back(ofTexture());
        colorized_ir_pixels.push_back(ofPixels());
        colorized_ir_textures.push_back(ofTexture());
    }
}

void MultiDeviceController::update()
{
    if (num_devices == 1)
    {
        ...
        k4a::image main_ir_image = captures[0].get_ir_image();

        // Convert k4a::image to ofPixels  & ofTexture
        k4a_color_image_to_OF(main_color_image, &capture_color_pixels[0], &capture_color_textures[0]);
        k4a_colorized_depth_image_to_OF(main_depth_image, &colorized_depth_pixels[0], &colorized_depth_textures[0], false);
        k4a_colorized_depth_image_to_OF(main_ir_image, &colorized_ir_pixels[0], &colorized_ir_textures[0], true);
    ...
    if (num_devices == 2)
    {
        ...
        k4a::image secondary_ir_image = captures[1].get_ir_image();

        // Convert k4a::image to ofPixels & ofTexture
        k4a_color_image_to_OF(secondary_color_image, &capture_color_pixels[1], &capture_color_textures[1]);
        k4a_colorized_depth_image_to_OF(secondary_depth_image, &colorized_depth_pixels[1], &colorized_depth_textures[1], false);
        k4a_colorized_depth_image_to_OF(secondary_ir_image, &colorized_ir_pixels[1], &colorized_ir_textures[1], true);
...
void MultiDeviceController::k4a_color_image_to_OF(k4a::image &img, ofPixels *pix, ofTexture *tex)
{
    ...
    if (!tex->isAllocated())
    {
        ...
        if (img.get_format() == K4A_IMAGE_FORMAT_COLOR_BGRA32)
        {
            tex->bind();
            {
                glTexParameteri(tex->texData.textureTarget, GL_TEXTURE_SWIZZLE_R, GL_BLUE);
                glTexParameteri(tex->texData.textureTarget, GL_TEXTURE_SWIZZLE_B, GL_RED);
            }
            tex->unbind();
        }
    }
...
void MultiDeviceController::k4a_colorized_depth_image_to_OF(k4a::image &img, ofPixels *pix, ofTexture *tex, bool grayscale)
{
    auto dims = glm::ivec2(img.get_width_pixels(), img.get_height_pixels());
    int num_channels = 4;
    ...
    auto range = GetDepthModeRange(main_config.depth_mode);
...
ofColor MultiDeviceController::colorize_grayscale(uint16_t &value, uint16_t &min, uint16_t &max)
{
...
ofColor MultiDeviceController::colorize_red2blue(uint16_t &value, uint16_t &min, uint16_t &max)
{
    if (value == 0)
    {
...
void MultiDeviceController::k4a_depth_image_to_OF(k4a::image &img, ofShortPixels *pix, ofTexture *tex)
{
    auto dims = glm::ivec2(img.get_width_pixels(), img.get_height_pixels());
    int num_channels = 1;
    ...
    pix->setFromPixels(reinterpret_cast<uint16_t *>(img.get_buffer()), dims.x, dims.y, num_channels);
...
void MultiDeviceController::draw()
{
    ...
    float aspect_ratio = capture_color_textures[0].getWidth() / capture_color_textures[0].getHeight();
    float x_spacing = (w - (num_devices + 1) * padding) / (num_devices + 0);
    float y_spacing = x_spacing / aspect_ratio;

    ofPushStyle();
    ofFill();
    for (int i = 0; i < num_devices; i++)
    {
        capture_color_textures[i].draw((i * x_spacing) + (i + 1) * padding, padding, x_spacing, y_spacing);
        ofDrawBitmapString("COLOR", (i * x_spacing) + (i + 1) * padding + 5, padding + 15);
        ofDrawBitmapString(("S/N: " + serial_numbers[i]), (i * x_spacing) + (i + 1) * padding + 5, padding + 35);
    }

    aspect_ratio = colorized_depth_textures[0].getWidth() / colorized_depth_textures[0].getHeight();
    float scaled_width = x_spacing / 2 - padding / 2;
    for (int i = 0; i < num_devices; i++)
    {
        colorized_depth_textures[i].draw((i * x_spacing) + (i + 1) * padding, y_spacing + 2 * padding, scaled_width, scaled_width);
        ofDrawBitmapString("DEPTH", (i * x_spacing) + (i + 1) * padding + 5, y_spacing + 2 * padding + 15);
        colorized_ir_textures[i].draw((i * x_spacing) + (i + 2) * padding + scaled_width, y_spacing + 2 * padding, scaled_width, scaled_width);
        ofDrawBitmapString("IR", (i * x_spacing) + (i + 2) * padding + scaled_width + 5, y_spacing + 2 * padding + 15);
    }
    ofPopStyle();
}
                                        ...
                                        double calibration_timeout)
{
    ...
    while (std::chrono::duration<double>(std::chrono::system_clock::now() - start_time).count() < calibration_timeout)
    {
        ...
        if (main_chessboard_corners_list.size() >= 1)
        {
