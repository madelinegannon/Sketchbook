...
void DeviceCapturer::get_capture()
{
    ...
    if (retrieve_ir)
    {
        ...
        if (image != NULL)
        {
            ...
            // image_data_ir = k4a_image_get_buffer(image);
            size_t size = k4a_image_get_size(image);
            if (image_data_ir == NULL)
                image_data_ir = (uint8_t*) malloc(size);
            memcpy(image_data_ir, k4a_image_get_buffer(image) , size);
...
void DeviceCapturer::colorize(k4a_image_t image, ofShortPixels *pix)
{
    ...
    for (int h = 0; h < height; ++h)
    {
        for (int w = 0; w < width; ++w)
        {
            ...
            {
                if (image_format == K4A_IMAGE_FORMAT_IR16)
                    pix->setColor(w, h, colorize_grayscale(&val, range.first, range.second));
                else if (image_format == K4A_IMAGE_FORMAT_DEPTH16)
                    pix->setColor(w, h, colorize_red2blue(&val, range.first, range.second));
            }
        }
    }
}
...
void DeviceCapturer::update_pixels_ir(k4a_image_t image)
{
    ...
    if (get_raw){
        pix->setFromPixels(reinterpret_cast<uint16_t *>(image_data_ir), dims.x, dims.y, num_channels);
        // pix->setFromPixels(reinterpret_cast<uint16_t *>(k4a_image_get_buffer(image)), dims.x, dims.y, num_channels);
    }
    else{
        colorize(image, pix);
